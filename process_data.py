...
def process_data(inpath = './data/flower_photos/',out_path='./data/process/',validation_percentage=10):
    class_names = get_class_names(inpath)#获取flower_datd的class_name
    print (class_names)
    ...
    for class_index, class_name in enumerate(class_names):
        ...
        tfrecordtrain = os.path.join(train_path+'/',class_name+'_train.rfrecords')#TFrecord数据集名字
        ...
        for file_index in file_indices:
            chance = np.random.randint(100)  # 随机产生100个数代表百分比train,test数据集分割
